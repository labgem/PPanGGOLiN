...
class TestIdenticalRegions:
    def test_init_with_valid_inputs(self, identical_rgps, families):
        ...
        identical_regions = IdenticalRegions("IdenticalRegions", identical_rgps, families, is_contig_border)
    ...
    def test_init_with_rgp_is_not_instance_region_in_identical_rgps(self, identical_rgps, families):
        """Tests that the IdenticalRegions object raise TypeError if one element is not instance Region.
        """
        with pytest.raises(TypeError):
            IdenticalRegions("IdenticalRegions", identical_rgps.union({1}), families, True)
    ...
    def test_init_with_family_is_not_instance_genefamilies_in_families(self, identical_rgps, families):
        """Tests that the IdenticalRegions object raise TypeError if one element is not instance Region.
        """
        with pytest.raises(TypeError):
            IdenticalRegions("IdenticalRegions", identical_rgps, families.union({1}), True)
    ...
    def test_eq_with_equal_identical_regions(self):
        ...
        identical_regions1 = IdenticalRegions("IdenticalRegions", identical_rgps1, families1, is_contig_border)
        identical_regions2 = IdenticalRegions("IdenticalRegions", identical_rgps2, families2, is_contig_border)
...
def test_dereplicate_rgp(identical_rgps):
    ...
    identical_region_obj = rgp_cluster.IdenticalRegions(name="identical_rgps_0",
...
def test_compute_rgp_metric(genes, families):
    ...
    expected_grr = (RGP_a.ID, RGP_b.ID, {'incomplete_aware_grr': shared_families / min(len(set(RGP_a.families)), len(set(RGP_b.families))),
    ...
    incomplete_aware_result = rgp_cluster.compute_rgp_metric(RGP_a, RGP_b, 0, "incomplete_aware_grr")
