...
The PPanGGOLiN `all` workflow generates a tab-separated file called `genome_statistics.tsv` describing the content of each genome used for building the pangenome.
It might be useful when working with fragmented data such as *MAGs* or in cases where there is a suspicion that some genomes might be chimeric or fall outside the intended taxonomic group (those genomes should be clear outliers in this file).
...
The `genome_statistics.tsv` file is described in full detail [here](../PangenomeAnalyses/pangenomeStat.md#genome-statistics-table).

#### Gene presence absence

The `all` workflow generates a presence absence matrix of genomes and gene families called `gene_presence_absence.Rtab`. This format mirrors the structure of the `gene_presence_absence.Rtab` file obtained from the pangenome analysis software [Roary](https://sanger-pathogens.github.io/Roary/).
...
#### Tile plot: detect pangenome structure and outliers
...
You can also find the *fitting* of the data to **Heaps' law**, which is usually used to represent this evolution of diversity in terms of gene families in each partition.


```{note}
The rarefaction curve is not drawn by default in the `all` subcommand as it requires heavy computation. 
