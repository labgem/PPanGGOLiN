...
def writeGeneSequences(pangenome, output, compress, genes, soft_core=0.95, show_bar=True):
...
def writeFastaGeneFam(pangenome, output, compress, gene_families, soft_core=0.95, show_bar=True):
...
def writeFastaProtFam(pangenome, output, compress, prot_families, soft_core=0.95, show_bar=False):
...
def writeRegionsSequences(pangenome, output, compress, regions, fasta, anno, show_bar=True):
    ...
    regions_to_write = sorted(regions_to_write, key=lambda
        x: x.organism.name)  # order regions by organism, so that we only have to read one genome at the time
...
def writeSequenceFiles(pangenome, output, fasta=None, anno=None, cpu=1, soft_core=0.95, regions=None, genes=None,
                       gene_families=None, prot_families=None, compress=False, show_bar=True):
    ...
    if gene_families is not None:
        writeFastaGeneFam(pangenome, output, compress, gene_families, soft_core=soft_core, show_bar=show_bar)
    if genes is not None:
        writeGeneSequences(pangenome, output, compress, genes, soft_core=soft_core, show_bar=show_bar)
    if regions is not None:
    ...
    if hasattr(args, "regions"):
        if args.regions is not None:
            if args.fasta is None and args.anno is None:
                raise Exception(
                    "The --regions options requires the use of --anno or --fasta (You need to provide the same file used to compute the pangenome)")
...
def launchSequences(args):
    ...
    writeSequenceFiles(pangenome, args.output, fasta=args.fasta, anno=args.anno, cpu=args.cpu, soft_core=args.soft_core,
                       ...
                       gene_families=args.gene_families, compress=args.compress, show_bar=args.show_prog_bars)
...
def writeSequenceSubparser(subparser):
    ...
    optional = parser.add_argument_group(
        title="Optional arguments. Indicating 'all' writes all elements. Writing a partition ('persistent', 'shell' or 'cloud') write the elements associated to said partition. Writing 'rgp' writes elements associated to RGPs.")
    ##could make choice to allow customization
    optional.add_argument("--regions", required=False, choices=["all", "complete"],
                          help="Write the RGP nucleotide sequences (requires --anno or --fasta used to compute the pangenome to be given)")
