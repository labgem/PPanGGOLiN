...
def launch(args):
    ...
    if args.anno:#if the annotations are provided, we read from it
        ...
        if args.clusters is None and pangenome.status["geneSequences"] == "No" and args.fasta_list is None and args.fasta_files is None:
        ...
        elif args.clusters is None and pangenome.status["geneSequences"] == "No" and args.fasta_files is not None:
...
def workflowSubparser(subparser):
    ...
    required.add_argument('--anno', required=False, type=str, help="A tab-separated file listing the organism names, and the gff filepath of its annotations (the gffs can be compressed). One line per organism. This option can be used alone IF the fasta sequences are in the gff files, otherwise the argument --fasta_list or --fasta_files needs to be used.")
