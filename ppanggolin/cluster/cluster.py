...
def firstClustering(sequences, tmpdir, cpu, code, coverage, identity, mode):
    ...
    cmd = ["mmseqs","cluster",seqdb, cludb, tmpdir.name ,"--cluster-mode", mode, "--min-seq-id", str(identity), "-c", str(coverage), "--threads", str(cpu), "--kmer-per-seq","80","--max-seqs","300"]
    logging.getLogger().debug(" ".join(cmd))
...
def clustering(pangenome, tmpdir, cpu , defrag = False, code = "11", coverage = 0.8, identity = 0.8, mode="0", force = False):
...
def launch(args):
    ...
    if args.clusters is None:
        clustering(pangenome, args.tmpdir, args.cpu, defrag=args.defrag, code=args.translation_table, coverage=args.coverage, identity=args.identity, mode=args.mode, force=args.force)
...
def clusterSubparser(subparser):
    ...
    optional.add_argument("--mode", required=False, default="0", choices=["0","1","2","3"], help = "the cluster mode of MMseqs2. 0: Setcover, 1: single linkage (or connected component), 2: CD-HIT-like, 3: CD-HIT-like (lowmem)")
