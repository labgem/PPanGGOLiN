...
def check_already_computed_metric(pangenome: Pangenome, genomes_fluidity: bool = False, print_metric:bool = True, recompute:bool = False) :
    """
    ...
    with tables.open_file(pangenome.file, "a") as h5f:
        ...
        if genomes_fluidity and 'genomes_fluidity' in info_group._v_attrs._f_list():
            logging.getLogger("PPanGGOLiN").warning("Genome fluidity has been already computed. "
...
def compute_metrics(pangenome: Pangenome, genomes_fluidity: bool = False, families_fluidity: bool = False, disable_bar: bool = False) -> dict:
    ...
    metrics_dict = {}
    if genomes_fluidity:
        metrics_dict['genomes_fluidity'] = compute_genomes_fluidity(pangenome, disable_bar)
...
def write_metrics(pangenome: Pangenome, metrics_dict: dict, print_metrics: bool = False):
    """
    ...
    with tables.open_file(pangenome.file, "a") as h5f:
        ...
        if 'genomes_fluidity' in metrics_dict.keys():
            logging.getLogger("PPanGGOLiN").info("Writing genome fluidity of the pangenome.")
...
def print_computed_metric(metrics_dict: dict):
    ...
    """
    metric_dict = {"Genomes_fluidity": {key:round(val,3) for key, val in metrics_dict.items()}}
    metric_yaml = yaml.dump(metric_dict, default_flow_style=False, sort_keys=False, indent=4)
    print(metric_yaml)
...
def launch(args: argparse.Namespace):
    ...
    logging.getLogger("PPanGGOLiN").debug("Check if one of the metrics was already computed")
    is_metric_already_computed =  check_already_computed_metric(pangenome, genomes_fluidity=args.genome_fluidity, print_metric=print_metrics, recompute=args.recompute_metrics)

    if not is_metric_already_computed or args.recompute_metrics:
        logging.getLogger("PPanGGOLiN").info("Metrics computation begin")
        metrics_dictionary = compute_metrics(pangenome, disable_bar=args.disable_prog_bar, genomes_fluidity=args.genome_fluidity)
...
def subparser(sub_parser: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    """
    parser = sub_parser.add_parser("metrics", formatter_class=argparse.RawTextHelpFormatter)
...
def parser_metrics(parser: argparse.ArgumentParser):
    ...
    required = parser.add_argument_group(title="Required arguments",
                                         description="Specify the required argument:")
    required.add_argument('-p', '--pangenome', required=False, type=Path,
                          help="Path to the pangenome .h5 file")

    onereq = parser.add_argument_group(title="Input file",
                                       description="Choose one of the following arguments:")
    ...
    optional.add_argument('--recompute_metrics', action="store_true",
